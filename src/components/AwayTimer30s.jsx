...
function playBeep(freq = 880, durationMs = 200) {
...
export default function AwayTimer30s() {
  const [enabled, setEnabled] = useState(true); // ✅ NEW
  ...
  const lastWarnedSecond = useRef(null);
  ...
  // Detect away
  useEffect(() => {
    const onMouseOver = () => setAway(false);
    const onMouseOut = (e) => {
      if (e.relatedTarget === null) setAway(true);
    };
    const onVisibility = () =>
  ...
  useEffect(() => {
    const shouldRun =
      enabled && away && remainingMs > 0 && !finished;
  ...
  // Final alarm
  useEffect(() => {
    if (enabled && remaining.totalSec === 0 && away && !finished) {
      setFinished(true);
      playBeep(600, 1000);
    }
  }, [enabled, remaining.totalSec, away, finished]);
